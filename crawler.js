...
// ===== 豆包一句话总结 =====
async function summarizeWithLLM(item, readme) {
    const key = getArkKey();
    if (!key) return null;
    // 喂 README 全文（已是纯文本）
                ...
                messages: [
                    {
                        role: "system",
                        content:
                            "你是 GitHub 仓库分析助手。用一句话（不超过30字，中文）总结这个项目主要解决什么问题。只输出总结本身，不要任何解释、前缀或标点装饰。",
                    },
                    { role: "user", content: payload },
                ],
                max_tokens: 300,
                temperature: 0.3,
                thinking: { type: "disabled" }, // 禁用思考链，避免简单总结耗时过长/超时
            },
            {
...
async function fillReadmeSummary(item) {
    // README：只要库中有非空即可复用，相同项目不重复拉取
    const cached = await pool.query(
        `SELECT readme, summary FROM trending_snapshots
    ...
    // summary：有 readme 但缺总结时才调用 LLM（README 已复用，无需重新抓取）
    if (item.readme && !item.summary) {
        item.summary = await summarizeWithLLM(item, item.readme);
    }
}
        ...
        for (const item of list) {
            await client.query(
                `INSERT INTO trending_snapshots
                 (dimension, language, title, links, description, repo_language, stars, forks, info, avatar, readme, summary, fetched_at)
...
module.exports = { runCrawl, pool, getData };
